...
class SpaceTimeBackboneParallel(nn3.SpaceTimeBackboneBase):
    def __init__(
            self,
            # structure
        ...
        assert num_layers_time == num_layers_space
        for index in range(num_layers_time):
            self.space_layers.add_module(
                name=f"parallel_layer_{index:02d}_space",
                module=nn2.CustomizedModernBertLayer(config=modernbert_config, layer_id=index, remove_rope=True),
            )
            self.time_layers.add_module(
                name=f"parallel_layer_{index:02d}_time",
    ...
    def forward_pretrain(
        ...
        # ======================================================
        # ======================================================
        x = self.forward_parallel_layers(x, attn_mask, d, t, o, e)
    ...
    def forward_fintune_dtu(
        ...
        # ======================================================
        # ======================================================
        x = self.forward_parallel_layers(x, attn_mask_u, d, t, u, e)
        # ======================================================
        # ======================================================
    ...
    def forward_parallel_layers(
            self,
            x_time: torch.Tensor,
            attn_mask_time: torch.Tensor,
            d: int,
            t: int,
            o: int,
            e: int,
    ):
        pos_id_space = torch.arange(o, device=x_time.device).unsqueeze(0)
        pos_id_time = torch.arange(t, device=x_time.device).unsqueeze(0)
        # ======================================================
        for layer_index, (space_layer, time_layer) in enumerate(zip(self.space_layers, self.time_layers)):
            # ======================================================
            layer_lambda = self.lambda_gates[layer_index].to(x_time.device)
            # ======================================================
            x_space, attn_mask_space = self.do_t_e__2__dt_o_e(x_time.clone(), attn_mask_time.clone(), d, t, o, e)
            assert x_time.shape == torch.Size([d * o, t, e])
            assert x_space.shape == torch.Size([d * t, o, e])
            altered_attn_mask_space, sliding_window_mask_space = self.update_attention_mask(attn_mask_space)
            x_space = space_layer(
                x_space,
                attention_mask=altered_attn_mask_space,
                sliding_window_mask=sliding_window_mask_space,
                position_ids=pos_id_space,
                output_attentions=False,
            )[0]
            x_space, _ = self.dt_o_e__2__do_t_e(x_space, attn_mask_space, d, t, o, e)
            # ======================================================
            altered_attn_mask, sliding_window_mask = self.update_attention_mask(attn_mask_time)
            x_time = time_layer(
                x_time,
                attention_mask=altered_attn_mask,
                sliding_window_mask=sliding_window_mask,
                position_ids=pos_id_time,
                output_attentions=False,
            )[0]
            # ======================================================
            x_time = layer_lambda * x_time + (1 - layer_lambda) * x_space
        # ======================================================
        return x_time
